...
void make_rwx(void * psc){
    if (mprotect((void*)psc, 1, PROT_READ| PROT_WRITE | PROT_EXEC) < 0) {
            put("mprotect failed: rwx");
            exit(-1);
        }
}

void make_rx(void * psc){
    if (mprotect((void*)psc, 1, PROT_READ | PROT_EXEC) < 0) {
