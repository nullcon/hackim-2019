...
invsbox = [0 for i in xrange(len(sbox))]

for idx, val in enumerate(sbox):
    invsbox[val] = idx

invp = [0 for i in xrange(len(p))]
...
givenct = toofun('ABCDEF','A'*BLOCK_LENGTH)

flagct = toofun('ABCDEF','B'*BLOCK_LENGTH)

enctable = {}

for i in xrange(256*256*256):
    enctable[fun(unhexlify("{:06x}".format(i)), 'A'*16)] = i
...
for i in xrange(256*256*256):
    dectable[unfun(unhexlify("{:06x}".format(i)), unhexlify(givenct))] = i
...
for i in enctable:
    if i in dectable:
        ...
        key = unhexlify("{:06x}".format(enctable[i]))+unhexlify("{:06x}".format(dectable[i]))

print unhexlify(unfun(key[:3], unhexlify(unfun(key[3:], unhexlify(flagct)))))